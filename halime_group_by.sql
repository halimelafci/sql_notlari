...
select urun_adi,max(urun_miktari) as max_miktar,min(urun_miktari)as min_miktar from manav  where urun_adi is not null
group by urun_adi order by max_miktar
...
select cinsiyet, avg(maas) from personel group by cinsiyet;
...
select ulke,sehir, count(sehir) from personel group by ulke,sehir;


-- Her ulke için bay ve bayan çalışan sayısı ve yaş ortalamasını sorgulayınız.

select cinsiyet, ulke, count(*) as calisan_sayisi, avg(yas) from personel group by cinsiyet, ulke;

-- Her ulke için bay ve bayan çalışan sayısı ve yaş ortalamasını  ve maası 30000 den büyük olanları sorgulayınız.

select cinsiyet, ulke, avg(yas),count(*) from personel where maas>30000 group by cinsiyet ,ulke;
...
select ulke, cinsiyet, round (avg(yas)) from personel where maas > 30000 group by ulke, cinsiyet order by round (avg(yas)) desc;
